...
public interface AppOrderRepository extends JpaRepository<AppOrder, Long> {
	...
	List<AppOrder> findAllByCreatedAtBetweenOrderByCreatedAtDesc(java.time.Instant from, java.time.Instant to, Pageable pageable);
	...
	@Query(value = """
		...
		order by week_start desc
		""", nativeQuery = true)
	List<WeeklyAdminStatsRow> findWeeklyAdminStatsRange(@Param("from") java.time.Instant from, @Param("to") java.time.Instant to);
}
